...
and copy_row_field :
  ...
  | From.Parsetree.Rtag (x0,x1,x2,x3) ->
    ...
    ; prf_attributes = copy_attributes x1
  ...
  | From.Parsetree.Otag (x0,x1,x2) ->
    {pof_desc =
      To.Parsetree.Otag (copy_loc (fun x -> x) x0,
                         copy_core_type x2);
     pof_attributes = copy_attributes x1;
     pof_loc = Location.none }
  | From.Parsetree.Oinherit x ->
    {pof_desc =
       To.Parsetree.Oinherit (copy_core_type x);
     pof_attributes = [];
     pof_loc = Location.none}
...
and copy_attribute :
  ...
  fun x  ->
    let (x0,x1) = x  in
    { attr_name = (copy_loc (fun x -> x) x0);
      attr_payload = (copy_payload x1);
...
and copy_type_extension :
  ...
  fun
     ...
     ->
    {
      ...
      To.Parsetree.ptyext_loc = Location.none
    }

and copy_type_exception :
  From.Parsetree.extension_constructor ->
  To.Parsetree.type_exception
  =
  fun x ->
    {
      ptyexn_constructor = copy_extension_constructor x;
      ptyexn_loc = Location.none;
      ptyexn_attributes= []; (* ... [@@id1] [@@id2] *)
    }
...
and copy_out_type :
  ...
  | From.Outcometree.Otyp_module (x0,x1,x2) ->
      To.Outcometree.Otyp_module
        (Oide_ident {printed_name = x0}, (List.map (fun x  -> x) x1),
          (List.map copy_out_type x2))
...
and copy_out_ident :
  ...
  | From.Outcometree.Oide_ident x0 -> To.Outcometree.Oide_ident { printed_name = x0 }
...
let copy_toplevel_phrase :
  ...
  | From.Parsetree.Ptop_dir (x0,x1) ->
    let a : To.Parsetree.directive_argument option = match x1 with
      | Pdir_none -> None
      | Pdir_string s -> Some { pdira_desc = Pdir_string s; pdira_loc = Location.none }
      | Pdir_int (s,m) -> Some { pdira_desc = Pdir_int (s,m); pdira_loc = Location.none }
      | Pdir_ident i -> Some { pdira_desc = Pdir_ident (copy_longident i); pdira_loc = Location.none }
      | Pdir_bool b ->  Some { pdira_desc = Pdir_bool b; pdira_loc = Location.none }
    in
    To.Parsetree.Ptop_dir
      {pdir_name = {txt = x0;loc= Location.none};
       pdir_arg = a;
