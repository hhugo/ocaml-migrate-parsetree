...
and copy_row_field :
  ...
  | From.Parsetree.Rtag (x0,x2,x3) ->
      To.Parsetree.Rtag
        (copy_loc copy_label x0,
         copy_attributes x.prf_attributes, copy_bool x2,
...
and copy_object_field :
  ...
  | From.Parsetree.Otag (x0,x2) ->
      To.Parsetree.Otag (copy_loc (fun x -> x) x0,
                         copy_attributes x.pof_attributes,
                         copy_core_type x2)
...
and copy_attribute :
  ...
  fun x  ->
    let (x0,x1) = x.attr_name, x.attr_payload  in
    ((copy_loc (fun x  -> x) x0),
      (copy_payload x1))
...
and copy_type_exception :
  From.Parsetree.type_exception -> To.Parsetree.extension_constructor =
  fun x ->
    copy_extension_constructor x.ptyexn_constructor
...
and copy_out_type :
  ...
  | From.Outcometree.Otyp_module (x0,x1,x2) ->
    let x0 = match x0 with
      | Oide_ident x -> x.printed_name
      | _ -> assert false
    in
...
and copy_out_ident :
  ...
  | From.Outcometree.Oide_ident x0 -> To.Outcometree.Oide_ident x0.printed_name
...
let copy_toplevel_phrase :
  ...
  | From.Parsetree.Ptop_dir x ->
    let arg : To.Parsetree.directive_argument = match x.pdir_arg with
      | None -> Pdir_none
      | Some a ->
        match a.pdira_desc with
        | (Pdir_string s) -> Pdir_string s
        | (Pdir_int (s,m)) -> Pdir_int (s,m)
        | (Pdir_ident l) -> Pdir_ident (copy_longident l)
